// ============================================
// CONTENT SCRIPT: HYBRID ROBUST FIX (v10.0)
// ============================================

console.log('ChatArchive: Hybrid Engine v10.0 Loaded');

// --- 1. IMAGE CAPTURE (Best of Both) ---

function captureImageElement(img) {
  // Relaxed checks from v5.0 (allow smaller images, don't filter as aggressively)
  if (!img || !img.complete || img.naturalWidth < 20) return null;
  if (img.src.includes('svg')) return null;

  try {
    const canvas = document.createElement('canvas');
    // Scale huge images down to safe dimensions (Max 1500px)
    ...
    // v9.0 Fix: White background for transparency
...
function parseDomToBlocks(root) {
  ...
  function traverse(node, indent = 0) {
    ...
    if (tag === 'pre') {
      ...
      let codeText = codeEl.innerText.replace(/[^\x09\x0A\x0D\x20-\x7E]/g, '');
    ...
    // IMAGES (Standard inline)
    if (tag === 'img') {
      const w = node.getAttribute('width') || node.naturalWidth;
      if (w > 40 && !node.src.includes('svg')) { // Relaxed check
...
// --- 4. EXTRACTOR (RESTORED v5.0 STRATEGY) ---
...
async function extractChatGPTContent() {
  ...
  for (const article of articles) {
    ...
    // 2. USER IMAGE SWEEP (The v5.0 Logic that worked)
    // ChatGPT user uploads are often separate from the text block.
    if (role === 'user') {
      const userImages = article.querySelectorAll('img');
      for (const img of userImages) {
        // Broad check for user uploaded images (often blobs)
        if (img.width > 50 && !img.src.includes('svg')) {
          // Check if it was already caught by parseDom
          const alreadyExists = blocks.some(b => b.type === 'image' && (b.base64?.length > 100));

          if (!alreadyExists) {
            const base64 = captureImageElement(img);
            if (base64) {
              // UNSHIFT: Put manual uploads at the TOP of the user's message
...
// --- 5. PDF GENERATOR (White Code Text) ---
...
async function generatePDF(data) {
  ...
  const COLORS = {
    ...
    header: [0, 0, 0],
    codeBg: [30, 30, 30],
    codeText: [255, 255, 255]   // Force White for Code
  };
  ...
  for (const msg of data.messages) {
    ...
    // Use narrower bubbles for better readability
...
// Mutation Observer to keep button alive
